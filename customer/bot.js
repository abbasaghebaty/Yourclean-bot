 ...
 */
async function editState(env, token, chatId, messageId, userId, state) {
  const view = getStateView(state, CONFIG);
  ...
  if (view.keyboardType === 'reply') {
    try {
       ...
       */
      console.error('editMessageReplyMarkup failed:', error);
    }
...
async function editFaqDetail(env, token, chatId, messageId, userId, index) {
  ...
  await setState(env, userId, `faq_${index}`);
...
export async function handleMessage(env, update) {
  ...
  const targetState = MENU_BUTTON_STATES[text];
...
export async function handleCallback(env, update) {
  ...
  if (data === 'main') {
    await clearState(env, userId);
    await editState(env, token, chatId, messageId, userId, 'main');
    return;
  }
  ...
  if (knownInlineStates.includes(data)) {
    await editState(env, token, chatId, messageId, userId, data);
    return;
  }

  if (data.startsWith('faq_')) {
    const index = Number.parseInt(data.slice(4), 10);

    if (!Number.isInteger(index) || index < 0 || index >= faq.length) {
      return;
    }

    await editFaqDetail(env, token, chatId, messageId, userId, index);
  }
}
